 ...
 * For any `.kt` source file in `src/commonMain/kotlin/`, all import statements SHALL reference
...
class CoreModuleImportConstraintPropertyTest {
    ...
    /**
     * Property 1: Scan all `.kt` files in `src/commonMain/kotlin/` and assert no forbidden imports exist.
     *
     * **Validates: Requirements 1.2, 8.1, 9.4**
     */
    @Test
    fun `core module source files contain no forbidden imports`() {
        val coreMainDir = findCoreCommonMainDir()
        val ktFiles = coreMainDir.walkTopDown()
            .filter { it.isFile && it.extension == "kt" }
            .toList()

        assertTrue(ktFiles.isNotEmpty(), "Expected to find .kt files in src/commonMain/kotlin/")
    ...
    private fun findCoreCommonMainDir(): File = File("src/commonMain/kotlin").also {
        check(it.isDirectory) {
            "Cannot locate src/commonMain/kotlin directory: ${it.absolutePath}"
        }
    }
}
