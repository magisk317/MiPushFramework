...
object UploadDataHelper {
    ...
    private fun pack(
        ...
        for (i3 in list.indices) {
            ...
            val payloadBytes = XmPushThriftSerializeUtils.convertThriftObjectToBytes(clientUploadDataItem)
            val length = payloadBytes?.size ?: 0
            if (length > i) {
                MyLog.e(
                ...
                if (i2 + length > i) {
                    ...
                    xmPushActionNotification.setBinaryExtra(IOUtils.gZip(
                        XmPushThriftSerializeUtils.convertThriftObjectToBytes(clientUploadData) ?: ByteArray(0)
                    ))
