...
public class PacketParserUtils {
        ...
        if ("1".equals(xmlPullParser.getAttributeValue("", "s"))) {
            ...
            while (true) {
                ...
                if (next != 2) {
                    ...
                    byte[] decodedSecurity = com.xiaomi.channel.commonutils.string.Base64Coder.decode(clientLoginInfoByChidAndUserId2.security);
                    byte[] packetIdBytes = attributeValue2.getBytes();
                    byte[] combinedKey = new byte[decodedSecurity.length + 1 + packetIdBytes.length];
                    System.arraycopy(decodedSecurity, 0, combinedKey, 0, decodedSecurity.length);
                    combinedKey[decodedSecurity.length] = (byte) '_';
                    System.arraycopy(packetIdBytes, 0, combinedKey, decodedSecurity.length + 1, packetIdBytes.length);
                    resetDecryptedMsgParser(rc4Decrypt(combinedKey, text));
    ...
    private static byte[] rc4Decrypt(byte[] key, String payload) {
        return rc4Transform(key, com.xiaomi.channel.commonutils.string.Base64Coder.decode(payload));
    }

    private static byte[] rc4Transform(byte[] key, byte[] payload) {
        ...
        int j = 0;
        for (int i = 0; i < 256; i++) {
            j = (j + (state[i] & 255) + (key[i % key.length] & 255)) & 255;
            byte tmp = state[i];
            state[i] = state[j];
            state[j] = tmp;
        }
        ...
        for (int idx = 0; idx < payload.length; idx++) {
            ...
            state[i] = state[j];
            state[j] = tmp;
            int keyByte = state[((state[i] & 255) + (state[j] & 255)) & 255] & 255;
            result[idx] = (byte) (payload[idx] ^ keyByte);
        }
        return result;
    }
}
