...
abstract class CollectionJob(
    ...
    companion object {
        ...
        private fun writeItemToFile(context: Context, dataCollectionItem: DataCollectionItem, secret: String) {
            val thriftBytes = XmPushThriftSerializeUtils.convertThriftObjectToBytes(dataCollectionItem)
            if (thriftBytes == null) return
            val encryptedData = CDataHelper.encryptData(secret, thriftBytes)
