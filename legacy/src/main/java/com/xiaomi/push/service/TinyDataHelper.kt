...
object TinyDataHelper {
    ...
    private fun generateUploadTinyDataNotification(
        ...
        return XmPushActionNotification("-1", false)
            ...
            .setAppId(appId)
            .setBinaryExtra(IOUtils.gZip(XmPushThriftSerializeUtils.convertThriftObjectToBytes(uploadData) ?: ByteArray(0)))
    ...
    fun pack(items: List<ClientUploadDataItem>?, packageName: String, appId: String?, maxSize: Int): ArrayList<XmPushActionNotification>? {
        ...
        for (item in items) {
            ...
            val actualSize = if (declaredSize > 0) declaredSize else XmPushThriftSerializeUtils.convertThriftObjectToBytes(item)?.size ?: 0
