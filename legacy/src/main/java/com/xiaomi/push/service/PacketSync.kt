...
class PacketSync(
    ...
    private fun handleBindResult(channelId: String, userId: String?, response: ChannelMessage.XMMsgBindResp) {
        ...
        val status = plan.clientStatus as? PushClientsManager.ClientStatus
        val notifyType = plan.notifyType
        if (status != null && notifyType != null) {
            client.setStatus(
                status,
                notifyType,
        ...
        val runtimeState = plan.runtimeState as? PushChannelState
    ...
    private fun handleKick(channelId: String, userId: String?, kickType: String?, kickReason: String?) {
        ...
        if (client != null) {
            ...
            val kickRuntimeState = plan.runtimeState as? PushChannelState
            if (kickRuntimeState != null) {
                pushAction.runtimeObserver.onChannelStateChanged(
                    client.pkgName,
                    channelId,
                    client.userId,
