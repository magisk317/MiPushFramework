...
class PushReconnectPolicyTest {

    @Test
    fun `initial reconnect has zero delay without prior connect`() {
        val plan = PushReconnectPolicy.computeDelayedReconnect(
            state = PushReconnectPolicy.initialState().copy(attempts = 1),
            nowMs = 1000L
        )
    ...
    fun `successful connect resets reconnect state`() {
        val state = PushReconnectPolicy.onConnectSucceeded(999L)
    ...
    fun `immediate reconnect increments attempts only when no pending job`() {
        val state = PushReconnectPolicy.initialState()
        val immediate = PushReconnectPolicy.planReconnect(
            state = state, forceImmediate = true, currentlyConnected = false,
            allowedByPolicy = true, hasPendingConnectJob = false, nowMs = 1000L
        )
        val replaceExisting = PushReconnectPolicy.planReconnect(
            state = state, forceImmediate = true, currentlyConnected = false,
            allowedByPolicy = true, hasPendingConnectJob = true, nowMs = 1000L
        )
    ...
    fun `delayed reconnect emits diagnostics on later attempts`() {
        val secondAttempt = PushReconnectPolicy.planReconnect(
            state = PushReconnectPolicy.initialState().copy(attempts = 1, lastConnectTime = 1000L),
            forceImmediate = false, currentlyConnected = false,
            allowedByPolicy = true, hasPendingConnectJob = false, nowMs = 2000L
        )
        val thirdAttempt = PushReconnectPolicy.planReconnect(
            state = PushReconnectPolicy.initialState().copy(attempts = 2, lastConnectTime = 1000L),
            forceImmediate = false, currentlyConnected = false,
            allowedByPolicy = true, hasPendingConnectJob = false, nowMs = 2000L
        )
