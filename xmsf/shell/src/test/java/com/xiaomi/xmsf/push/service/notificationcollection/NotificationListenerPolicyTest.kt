...
class NotificationListenerPolicyTest {
    @Test
    fun `accepts only concrete current user notifications`() {
        assertTrue(NotificationListener.acceptsUser(eventUserId = 0, currentUserId = 0))
        assertFalse(NotificationListener.acceptsUser(eventUserId = -1, currentUserId = 0))
        assertFalse(NotificationListener.acceptsUser(eventUserId = 999, currentUserId = -1))
    }
