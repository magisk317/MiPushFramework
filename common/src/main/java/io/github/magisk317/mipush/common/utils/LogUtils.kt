...
object LogUtils {
    ...
    fun clearLog(context: Context) {
        ...
        if (!result.success) {
            Toast.makeText(
                context,
                context.getString(io.github.magisk317.mipush.common.R.string.log_share_error, result.details),
                Toast.LENGTH_SHORT,
            ).show()
            return
        }
        Toast.makeText(context, context.getString(io.github.magisk317.mipush.common.R.string.log_clear_done), Toast.LENGTH_SHORT).show()
    }
    ...
    @JvmStatic
    fun pruneModuleLogsForToday(moduleLogDir: File, now: Date = Date()) {
        pruneDailyFiles(moduleLogDir, currentDateString(now), dailyModuleLogPattern)
    }

    fun pruneDailyFiles(logDir: File, currentDate: String, pattern: Regex) {
        runCatching {
