...
object NotificationProgressTextSupport {
    ...
    fun extractProgressPercent(text: CharSequence?): Int {
        val match = progressPattern.find(text?.toString().orEmpty()) ?: return NO_PROGRESS
    ...
    fun extractCountdownMillis(title: CharSequence?, content: CharSequence?): Long {
        return extractDurationMillis("${title?.toString().orEmpty()} ${content?.toString().orEmpty()}")
    }
    ...
    fun resolveAlertHint(title: CharSequence?, content: CharSequence?): String? {
        val text = "${title?.toString().orEmpty()} ${content?.toString().orEmpty()}"
        return when {
    ...
    private fun durationPattern(units: String): Regex {
        return Regex("(\\d+)\\s*(?:$units)", RegexOption.IGNORE_CASE)
    }
