...
open class MiPushFacadeService : Service() {
    @Inject lateinit var configCenter: io.github.magisk317.mipush.runtime.app.ConfigCenter
    ...
    private fun handleRuntimeIntent(intent: Intent) {
        if (intent.component?.className == Constants.XM_PUSH_SERVICE_CLASS) {
    ...
    private fun forwardToPushServiceMain(intent: Intent) {
        val intent2 = Intent(intent.action ?: "com.xiaomi.push.service.ACTION_START").apply {
