...
@RequiresApi(M) class NotificationsRevivalForSelfUpdated(private val context: Context, private val isTargetNotification: (StatusBarNotification) -> Boolean): BroadcastReceiver() {
    ...
    private fun timeout() = if (isDebuggable(context)) TIMEOUT_DEBUG else TIMEOUT

    private fun isDebuggable(context: Context) = 0 != (context.applicationInfo.flags and ApplicationInfo.FLAG_DEBUGGABLE)
    ...
    private val mInstallSessionCallback = object: PackageInstaller.SessionCallback() {
        ...
        override fun onFinished(sessionId: Int, success: Boolean) {
            if (isDebuggable(context) && ! success && isSessionForOurPackage(sessionId))    // For debugging convenience
