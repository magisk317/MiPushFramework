...
object TinyDataHelper {
    ...
    fun nextTinyDataItemId(): String {
        synchronized(TinyDataHelper::class.java) {
            ...
            return today + Constants.ACCEPT_TIME_SEPARATOR_SERVER + idGen.incrementAndGet()
        }
    }
    ...
    fun pack(items: List<ClientUploadDataItem>?, packageName: String, appId: String?, maxSize: Int): ArrayList<XmPushActionNotification>? {
        ...
        for (item in items) {
            ...
            if (extra != null && extra.containsKey(ReportConstants.UPLOAD_DATA_ITEM_EXTRA_KEY_SIZE)) {
                val sizeValue = extra[ReportConstants.UPLOAD_DATA_ITEM_EXTRA_KEY_SIZE]
                declaredSize = sizeValue?.toIntOrNull() ?: 0
                if (extra.size == 1) {
                    item.setExtra(null)
                } else {
                    extra.remove(ReportConstants.UPLOAD_DATA_ITEM_EXTRA_KEY_SIZE)
                }
            }
