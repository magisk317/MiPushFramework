...
class ServiceConfig private constructor() {
    ...
    companion object {
        ...
        fun getDeviceUUID(): String? {
            synchronized(ServiceConfig::class.java) {
                if (deviceUUID == null) {
                    val sharedPreferences = SystemUtils.getContext().getSharedPreferences(PREF_NAME, 0)
                    deviceUUID = sharedPreferences.getString(PREF_UUID, null)
                    if (deviceUUID == null) {
                        deviceUUID = DeviceInfo.getDeviceId(SystemUtils.getContext(), false)
                        if (deviceUUID != null) {
    ...
    private fun fetchConfig() {
        ...
        val task = object : SerializedAsyncTaskProcessor.SerializedAsyncTask() {
            ...
            override fun process() {
                try {
                    val region = AppRegionStorage.getInstance(SystemUtils.getContext()).getRegion()
                    ...
                    val remoteConfig = ChannelConfig.PushServiceConfig.parseFrom(
                        Base64.decode(HttpUtils.get(SystemUtils.getContext(), url, null), 10),
                    )
    ...
    private fun load() {
        ...
        try {
            input = BufferedInputStream(SystemUtils.getContext().openFileInput(CLOUDCONFIG))
    ...
    private fun save() {
        try {
            ...
            val output = BufferedOutputStream(SystemUtils.getContext().openFileOutput(CLOUDCONFIG, 0))
            output.use {
    ...
    fun getBoolSetting(key: String, defaultValue: Boolean): Boolean {
        return SystemUtils.getContext().getSharedPreferences(PREF_NAME, 0).getBoolean(key, defaultValue)
    }
    ...
    fun setSetting(key: String, value: Boolean) {
        SystemUtils.getContext().getSharedPreferences(PREF_NAME, 0).edit().putBoolean(key, value).commit()
    }
}
