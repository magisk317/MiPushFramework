...
object MIPushMessageRoutingSupport {
    ...
    fun shouldSendBroadcast(
        ...
        if (extra != null &&
            extra.containsKey(PushConstants.EXTRA_PARAM_CHECK_ALIVE) &&
            ...
            val isAppRunning = AppInfoUtils.isAppRunning(pushService.applicationContext, packageName)
            notification.extra.put(PushConstants.EXTRA_PARAM_APP_RUNNING, isAppRunning.toString())
            if (!isAppRunning) {
                val awakened = extra[PushConstants.EXTRA_PARAM_AWAKE].toBoolean()
                notification.extra.put(PushConstants.EXTRA_PARAM_AWAKED, awakened.toString())
                shouldSend = awakened
            }
            try {
