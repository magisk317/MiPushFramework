...
import com.xiaomi.push.service.PushChannelState as LegacyPushChannelState
...
import com.xiaomi.push.service.PushRegistrationState as LegacyPushRegistrationState
...
import com.xiaomi.xmsf.runtime.PushChannelState as RuntimePushChannelState
import com.xiaomi.xmsf.runtime.PushConnectionState as RuntimePushConnectionState
import com.xiaomi.xmsf.runtime.PushRegistrationState as RuntimePushRegistrationState
...
import io.github.magisk317.mipush.framework.network.PushHostManagerCompat
import io.github.magisk317.mipush.framework.notification.MyMIPushNotificationHelper
import io.github.magisk317.mipush.framework.pipeline.MiPushRuntimeBridge
import io.github.magisk317.mipush.framework.sdk.PushMessageProcessor
import io.github.magisk317.mipush.framework.sdk.PushMessageProcessorEntryPoint
...
import io.github.magisk317.mipush.framework.PushPacketSyncRuntime
...
class MiPushRuntimeObserverBridge(private val context: Context) : IPushRuntimeObserver {
    ...
    private fun frameworkProcessor(): PushMessageProcessor {
        ...
        ).frameworkPushMessageProcessor()
    }

    private fun toRuntimeChannelState(state: LegacyPushChannelState): RuntimePushChannelState {
        return when (state) {
            LegacyPushChannelState.Unbound -> RuntimePushChannelState.Unbound
            LegacyPushChannelState.Binding -> RuntimePushChannelState.Binding
            LegacyPushChannelState.Bound -> RuntimePushChannelState.Bound
            LegacyPushChannelState.Unbinding -> RuntimePushChannelState.Unbound
            LegacyPushChannelState.Closed -> RuntimePushChannelState.Closed
            LegacyPushChannelState.Kicked -> RuntimePushChannelState.Kicked
            LegacyPushChannelState.OpenFailed -> RuntimePushChannelState.OpenFailed
        }
    }

    private fun toRuntimeRegistrationState(state: LegacyPushRegistrationState): RuntimePushRegistrationState {
        return when (state) {
            LegacyPushRegistrationState.Unregistered -> RuntimePushRegistrationState.Unregistered
            LegacyPushRegistrationState.Registering -> RuntimePushRegistrationState.Registering
            LegacyPushRegistrationState.Registered -> RuntimePushRegistrationState.Registered
            LegacyPushRegistrationState.Unregistering -> RuntimePushRegistrationState.Unregistered
            LegacyPushRegistrationState.Failed -> RuntimePushRegistrationState.Failed
        }
    }

    private fun toRuntimeConnectionState(stateName: String): RuntimePushConnectionState {
        return when (stateName) {
            "Connected", ConnectionStatus.connected.name -> RuntimePushConnectionState.Connected
            "Connecting", ConnectionStatus.connecting.name -> RuntimePushConnectionState.Connecting
            else -> RuntimePushConnectionState.Disconnected
        }
    }
    ...
    override fun reconnectionFailed(connection: Connection, error: Exception) {
        ...
        PushRuntime.observeConnectionState(
            state = RuntimePushConnectionState.Disconnected,
    ...
    override fun reconnectionSuccessful(connection: Connection) {
        ...
        PushRuntime.observeConnectionState(
            state = RuntimePushConnectionState.Connected,
    ...
    override fun connectionStarted(connection: Connection) {
        ...
        PushRuntime.observeConnectionState(
            state = RuntimePushConnectionState.Connecting,
    ...
    override fun createHostManager(context: Context, hostFilter: Any?, httpGet: Any?, userId: String): Any? {
        ...
        return PushHostManagerCompat.create(context, filter, getter, userId)
    }
    ...
    override fun onRegistrationStateChanged(packageName: String, state: LegacyPushRegistrationState, reason: String, message: String) {
        PushRuntime.observeRegistrationState(packageName, toRuntimeRegistrationState(state), reason, message)
    }
    ...
    override fun observeUnregistration(packageName: String, state: LegacyPushRegistrationState) {
    ...
    override fun attachAccountClient(client: Any) {
        ...
        io.github.magisk317.mipush.framework.lifecycle.runtime.PushAccountRuntime.attachAccountClient(
            pushAction = pushAction,
            context = appContext,
            account = account,
            source = "MiPushRuntimeObserverBridge.attachAccountClient"
        )
    }
    ...
    override fun onChannelStateChanged(
        ...
        PushRuntime.observeChannelState(
            ...
            userId = userId,
            session = session,
            state = toRuntimeChannelState(state),
            source = reason,
    ...
    override fun shouldNotifyClient(
        ...
        return io.github.magisk317.mipush.framework.lifecycle.support.PushClientStatusSupport.shouldNotifyClient(
            client,
            type,
            reasonCode,
            errorType
    ...
    override fun computeNotifyDelay(
        ...
        return io.github.magisk317.mipush.framework.lifecycle.support.PushClientStatusSupport.computeNotifyDelay(client).toLong()
    }
    ...
    override fun onClientStatusChanged(client: Any, type: Int, reasonCode: Int, reasonMessage: String?, errorType: String?) {
        ...
        io.github.magisk317.mipush.framework.lifecycle.support.PushClientStatusSupport.notifyClientStatus(
            info,
            type,
            reasonCode,
            reasonMessage,
    ...
    override fun constructBindBlob(client: Any): Blob? {
        return (client as? PushClientsManager.ClientLoginInfo)?.let {
            io.github.magisk317.mipush.framework.lifecycle.support.PushClientsStateSupport.constructBindBlob(it)
        }
    }

    override fun constructUnbindBlob(chid: String, userId: String): Blob? {
        return io.github.magisk317.mipush.framework.lifecycle.support.PushClientsStateSupport.constructUnbindBlob(chid, userId)
    }
    ...
    override fun removeCachedMsgId(msgId: String) {
        frameworkProcessor().removeCachedMsgId(msgId)
    }
    ...
    override fun processMIPushIntent(intent: Intent): Any? {
        return frameworkProcessor().processIntent(appContext, intent)
    }
    ...
    override fun resolveReconnectAttemptPlan(
        ...
        return PushReconnectRuntime.planReconnect(state, force, isConnected, hasReconnectionJob, System.currentTimeMillis())
    }
