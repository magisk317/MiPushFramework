package io.github.magisk317.mipush.framework.pipeline
...
object MiPushRuntimeBridge {
    ...
    fun onApplicationIntentReceived(context: Context, intent: Intent?) {
        ...
        runCatching {
            io.github.magisk317.mipush.platform.support.GlobalSingletons.miPushEventListener().receiveFromApplication(intent)
    ...
    fun onIntentForwardedToServer(intent: Intent?) {
        if (intent == null) return
        runCatching {
            io.github.magisk317.mipush.platform.support.GlobalSingletons.miPushEventListener().transferToServer(intent)
        }.onFailure {
    ...
    fun onPayloadFromServer(
        ...
        runCatching {
            io.github.magisk317.mipush.platform.support.GlobalSingletons.registrationRecorder().initContext(context.applicationContext)
            io.github.magisk317.mipush.platform.support.GlobalSingletons.registrationRecorder().recordRegSec(container)
        }.onFailure {
            logger.e("recordRegSec failed source=$source", it)
        }
        runCatching {
            io.github.magisk317.mipush.platform.support.GlobalSingletons.miPushEventListener().receiveFromServer(container)
        }.onFailure {
    ...
    fun onTransferToApplication(container: XmPushActionContainer?) {
        ...
        runCatching {
            io.github.magisk317.mipush.platform.support.GlobalSingletons.miPushEventListener().transferToApplication(container)
        }.onFailure {
