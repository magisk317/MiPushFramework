...
class PushMessageProcessor @Inject constructor(
    ...
    private val MAX_MSG_CACHE_COUNT = 25
    ...
    /**
     * Legacy processIntent logic migrated from PushMessageProcessor.java
     */
    fun processIntent(context: Context, intent: Intent): Any? {
        if (PushConstants.MIPUSH_ACTION_NEW_MESSAGE != intent.action) {
            return null
        }
        return runCatching {
            val processorClass = Class.forName("com.xiaomi.mipush.sdk.PushMessageProcessor")
            val getInstance = processorClass.getMethod("getInstance", Context::class.java)
            val processor = getInstance.invoke(null, context)
            val processIntent = processorClass.getDeclaredMethod("processIntent", Intent::class.java)
            processIntent.isAccessible = true
            processIntent.invoke(processor, intent)
        }.onFailure {
            logger.e("processIntent failed", it)
        }.getOrNull()
    }

    fun removeCachedMsgId(messageId: String) {
...
interface PushMessageProcessorEntryPoint {
    fun frameworkPushMessageProcessor(): PushMessageProcessor
}
