...
ci_variable_ref="$(sed -nE 's/^[[:space:]]*MAGISK_CI_TOOLKIT_REF:[[:space:]]*"?([0-9a-fA-F]{40}|v[0-9]+)"?[[:space:]]*$/\1/p' "$CI_FILE")"
...
[[ "$ci_variable_ref" =~ $SHA_PATTERN ]] || fail "MAGISK_CI_TOOLKIT_REF must be a full SHA or a v<n> tag"
...
for ref in "${include_refs[@]}"; do
  ...
  [[ "$normalized_ref" =~ $SHA_PATTERN ]] || fail "include ref must be a full SHA or a v<n> tag: $ref"
...
resolver_ref="$(sed -nE 's/^TOOLKIT_REF="\$\{MAGISK_CI_TOOLKIT_REF:-([0-9a-fA-F]{40}|v[0-9]+)\}"$/\1/p' "$RESOLVER_FILE")"
[ -n "$resolver_ref" ] || fail "no resolver fallback SHA found in $RESOLVER_FILE"
resolver_ref="$(printf '%s' "$resolver_ref" | normalize_sha)"
[[ "$resolver_ref" =~ $SHA_PATTERN ]] || fail "resolver fallback must be a full SHA or a v<n> tag"
[ "$resolver_ref" = "$ci_variable_ref" ] ||
  fail "resolver fallback $resolver_ref differs from MAGISK_CI_TOOLKIT_REF $ci_variable_ref"

action_ref="$(sed -nE 's/^[[:space:]]*default: .([0-9a-fA-F]{40}|v[0-9]+).$/\1/p' "$ACTION_FILE")"
