...
class PersonalAssistantDetailHook : BaseHook() {
        ...
        runCatching {
            ...
            fragment.hookMethod("dataLoadingStatus", stateClass) {
                doBefore {
                    ...
                    if (code != API_CODE_NO_ADAPTER_VERSION && code != API_CODE_NO_ADAPTER_CONTENT) return@doBefore
                    val vm = runCatching {
                    ...
                    runCatching {
                        ...
                        args[0] = stateSuccessClass.getDeclaredConstructor(java.lang.Boolean.TYPE)
                            .apply { isAccessible = true }
                            .newInstance(true)
    ...
    private fun renderMockPreview(
        ...
        if (isRecent) {
            ...
            val tertiary = pkgContext.getColor(id("widget_text_tertiary", "color"))
            val rows = listOf(
    ...
    private fun buildWrapper(classLoader: ClassLoader, icon: Drawable?): Any {
        ...
        fun response(widget: Any, uniqueCode: String, originStyle: Int): Any {
            val ctor = responseClass.getDeclaredConstructor(
                ...
                Long::class.javaObjectType,       // priceInCent
            ...
            return ctor.newInstance(
                ...
                "MiPush",
                "1.0.4",
                PersonalAssistantPickerHook.MIPUSH_VERSION_CODE,
                "",
                "1.0.4",
                "",
                "",
        ...
        return wrapperCtor.newInstance(
            responses,                          // pickerDetailResponses
            MIPUSH_PACKAGE,                     // appPackage
            "",                                 // appIcon
            "MiPush",                           // appName
            PersonalAssistantPickerHook.MIPUSH_VERSION_CODE, // appVersionCode
            "1.0.4",                            // appVersionName
            "",                                 // appDownloadUrl
            "",                                 // appPublisherName
            "",                                 // appPermissionUrl
            "",                                 // appPrivacyUrl
            tip,                                // installInfo
            ArrayList<Any>(),                   // mamlsNeedDownload
            false,                              // z8
            0,                                  // i11
            icon,                               // localAppIcon
            false,                              // useAppIcon
    ...
    companion object {
        ...
        /** 小部件中心接口失败码：未审核包无适配版本内容（impl have no adapter version content）。 */
        private const val API_CODE_NO_ADAPTER_VERSION = 32007

        /** 小部件中心接口失败码：未审核包无适配内容，与 32007 同族的失败渲染路径。 */
        private const val API_CODE_NO_ADAPTER_CONTENT = 32006

        /** mock 预览二次复用的延迟（毫秒）；首次复用的 600ms 在 detekt 忽略清单内，保持内联。 */
        private const val MOCK_PREVIEW_REAPPLY_DELAY_MS = 1500L
